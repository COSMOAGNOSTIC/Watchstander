...
| `is_over_side` mislabeled as "overhead" in rationale text | The conflict is still correctly (conservatively) flagged, but the human-facing rationale calls an over-the-side package "Overhead work," which is geometrically backwards. Quantified as `eval/scenarios.py`'s `rationale-over-side-labeled-overhead` |
