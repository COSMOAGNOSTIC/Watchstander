...
- [x] NAVSEA 8010 procedural citations verified against primary-source text, `MAX_CONCURRENT_HOT_WORKERS_PER_FIRE_WATCH` corrected from `1` to the manual's actual limit of `4` (2026-08-08, ADR-023) — all seven `navsea_8010_psns_v2014.json` entries flip from `verified: false` to `verified: true`; two Known Debt rows closed. One new, real (but lower-priority, over-flagging-direction) gap found and disclosed, not fixed: `_fire_watch_capacity_conflicts()`'s size gate checks raw per-fire-watch package count, not per-time-window concurrency — see ARCHITECTURE.md Known Debt. 80/80 tests passing (up from 78)
